...
import { CardPessoas } from "@/component/pessoas/frament/card";
...
function Pessoas() {
  ...
  useEffect(() => {
    (async () => {
      setLoading(true)
      const response = await fetch('/api/pessoas/get');
  ...
  const empresaFilter: any = Pessoas.filter((E: any) => {
        ...
        <Box h={"95%"} bg="#edf3f8" overflow={"auto"}>
          <Flex py={50} flexWrap={'wrap'} gap='' justifyContent={"center"} px={5} w="full">
            <Suspense fallback={<div>Carregando dados...</div>}>
              <CardPessoas data={empresaFilter} />
            </Suspense>
          </Flex>
        </Box>
      </Flex>
